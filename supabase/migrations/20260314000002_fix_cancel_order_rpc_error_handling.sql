-- ============================================================
-- cancel_order_with_refund: 에러 핸들링 강화 + 정산 감산 안전 처리
--
-- 수정 사항:
-- 1. EXCEPTION 블록에서 실제 SQLSTATE/SQLERRM을 jsonb 응답에 포함 (디버깅 가능)
-- 2. 정산 감산 FOR LOOP 내 settlement 조회 시 NOT FOUND 방어
-- 3. 정산 감산 SELECT INTO 시 NULL 안전 처리
-- ============================================================
...
BEGIN
  ...
  -- settlement_gift_items에 관련 데이터가 없으면 UPDATE는 0행 영향 (정상)
  ...
  LOOP
    -- 해당 settlement의 현재 상태 조회 (존재 여부 방어)
    v_settlement_found := false;
    SELECT status, COALESCE(memo, '')
    INTO v_settlement_status, v_existing_memo
    FROM settlements
    WHERE id = v_settlement_rec.settlement_id;

    IF FOUND THEN
      v_settlement_found := true;
    END IF;

    -- settlement가 존재하지 않으면 건너뜀 (데이터 무결성 오류 방어)
    IF NOT v_settlement_found THEN
      RAISE WARNING '[cancel_order_with_refund] settlement not found: %', v_settlement_rec.settlement_id;
      CONTINUE;
    END IF;
...
EXCEPTION
  WHEN OTHERS THEN
    -- 에러 상세를 WARNING으로 기록 + 응답 jsonb에도 포함 (디버깅용)
